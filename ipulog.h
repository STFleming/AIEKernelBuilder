class IPULogger{
	...
	~IPULogger(){
        ...
        void log_peel_params(uint32_t *hmsg, uint32_t* cnt){{ return; }}

        // general case: peels the parameters off the argument list and appends them to the message
        template<typename P1, typename ... Param>
        void log_peel_params(uint32_t *hmsg, uint32_t *cnt, const P1& p1, Param& ... param){
            hmsg[*cnt+1] = *((uint32_t*)&p1); // prune type and place raw bytes in the host message
            *cnt = *cnt + 1; 
        ...
        void write(const char *msg, const Param& ... param) {
            if (almost_full(16)) {
		_write("Log buffer is full -- we have dropped %u messages", _dropped_msgs++);
		_buffer -= 8;
		_count -= 8;
                return;
            }
	...
	void _write(const char *msg, const Param& ... param) {
        ...
        bool almost_full(uint32_t amount){
            return _count >= (_maxlen - amount);
        }
